...
-Rush&Fish- 
 ...
 • For Loop 노드를 이용하여 맵타일을 무한으로 생성하였고 Inrange 노드를 이용하여 
   확률로 Object를 스폰 해주는 기술을 구현하였다.
 • 많은 Object 상호 작용으로 인해 많은 변수의 알고리즘이 관건이었던 프로젝트라 
   GameMode 블루프린트를 적극 활용하였다.
 ...
 • 하지만…. 첫 번째 프로젝트와 같이 Custom Event, Event Dispatcher 등의 기능들을
   ...
   가져왔을 거라 생각한다.
 • 프로젝트 진행 당시 프로그램 코드 개발보다 패키징 설정에 애를 먹었었다. 안드로이드
   스튜디오라는 보조 프로그램을 활용해서 모바일 패키징 설정을 잡다 보니 초기 설정이
   가장 큰 걸림돌이 되었다. 이를 해결하기 위해 언리얼 엔진 공식문서를 참조하였으며 
   그 외 수많은 구글링을하여 문제(언리얼 엔진과 안드로이드 스튜디오뿐만 아니라 컴퓨터
   ...
   찾아내지 못하고 아쉽게도 프로젝트를 종료하였다. 
 • 초기 환경 설정의 중요성을 크게 깨닫게 되는 프로젝트였고 모바일 플랫폼으로 개발을 
   다시 한다면 세팅 환경부터 다시 확실히 다져 놓고 작업을 들어갈 생각이다.
...
-Matador-
 ...
 • UI 및 Object 상호작용을 담당 하였으며 첫 프로젝트라 언리얼 관련 구글링에 기초를
   배울수 있게된 기간이라 생각한다.
 • Possession 노드를 활용하여 빙의할 수 있는 좀비를 구현 하였다.
 • Custom Event, Event Dispatcher, 등의 기능들을 활용 할 줄 몰랐었을 때라 
   ...
   이을킬 수 있는 개발 이었다 생각한다.
 • 위 사실은 두번째 프로젝트가 끝이 날쯤 알게 되었으며 3번째 프로젝트 부터 같은 실수를 
   반복 하지 않기 위해 이벤트 관련 노드들 중점으로 찾아서 작업을 하게 되었다.
 • 시간적 여유가 된다면 VR 플렛폼으로 다시 개발 해 보고 싶은 프로젝트다.
							</pre>
				</div>
